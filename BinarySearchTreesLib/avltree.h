...
template <typename K, typename V> class AVLTree final : public BinarySearchTree<K, V>
{
public:
...
template <typename K, typename V> AVLTree<K, V>::AVLTree(AVLTree&& sourceTree)
{
    AVLTree::_moveAssignTree(sourceTree);
}

template <typename K, typename V> AVLTree<K, V>& AVLTree<K, V>::operator=(const AVLTree& sourceTree)
{
    if (this != &sourceTree)
    {
        AVLTree::clear();
...
template <typename K, typename V> AVLTree<K, V>& AVLTree<K, V>::operator=(AVLTree&& sourceTree)
{
    if (this != &sourceTree)
    {
        AVLTree::clear();
...
/* Steps involved:
   ...
   - if removed node is not root:
     ...
     - recursively balance resulting unbalanced nodes (sub-trees) starting with parent and going bottom-up until
   reaching root
*/
template <typename K, typename V>
typename BinarySearchTree<K, V>::spNode AVLTree<K, V>::_removeSingleChildedOrLeafNode(
    typename BinarySearchTree<K, V>::spNode nodeToRemove)
{
    ...
    if (nodeToRemove)
    {
        assert((!nodeToRemove->getLeftChild() || !nodeToRemove->getRightChild()) &&
               "Node to be removed has more than one child");
        parent = dynamic_pointer_cast<AVLNode>(
            nodeToRemove->getParent()); // store parent in advance (TBinarySearchTree::_removeSingleChildedOrLeafNode()
                                        // decouples from parent)

        /* reason for using (void): the replacing node is not relevant as the removed node parent will be the first node
           to be checked whether being unbalanced (no matter if removed node is leaf or has one child */
    ...
    for (spAVLNode currentNode{parent}; currentNode != nullptr;)
    {
        ...
        if (!currentNodeChild)
        {
            assert(false && "Null larger height child found for unbalanced node"); // defensive programming, normally
        ...
        {
            assert(false &&
                   "Null larger height grandchild found for unbalanced node"); // defensive programming, normally this
...
template <typename K, typename V>
typename AVLTree<K, V>::spAVLNode AVLTree<K, V>::_balanceSubtree(spAVLNode grandparent, spAVLNode parent,
                                                                 spAVLNode child)
{
    ...
    {
        assert(child->getParent() == parent && parent->getParent() == grandparent &&
               "Invalid relationships detected between subtree nodes");

        // this node reference is required by the node removal operation as after balancing it will recur starting from
        // resulting root in search for another unbalanced node (search stops when reaching TREE ROOT)
        resultingSubtreeRoot = parent;

        // step 1: do the corresponding rotations based on the subtree topology/relationships between the three nodes in
        // order to obtain a balanced subtree (restore AVL rules)
        if (parent->isLeftChild() && child->isLeftChild()) // left - left case (rotate grandparent right)
        {
            AVLTree::_rotateNodeRight(grandparent);
        }
        else if (parent->isLeftChild() &&
                 child->isRightChild()) // left - right case (rotate parent left and then grandparent right)
        {
...
AVLTree<K, V>::AVLNode::AVLNode(const K& key, const V& value)
    ...
    , m_Height{1}
// the subtree height includes the node itself (we assume it is not connected to anything when first created)
{
}

template <typename K, typename V> void AVLTree<K, V>::AVLNode::updateHeight()
{
...
template <typename K, typename V> bool AVLTree<K, V>::AVLNode::isBalanced() const
{
...
typename AVLTree<K, V>::AVLNode::spAVLNode AVLTree<K, V>::AVLNode::getGreaterHeightChild() const
{
    spAVLNode const leftChild{std::dynamic_pointer_cast<AVLNode>(AVLNode::getLeftChild())};
    spAVLNode const rightChild{std::dynamic_pointer_cast<AVLNode>(AVLNode::getRightChild())};
    spAVLNode const result{!leftChild || (rightChild && leftChild->m_Height < rightChild->m_Height) ? rightChild
                                                                                                    : leftChild};
